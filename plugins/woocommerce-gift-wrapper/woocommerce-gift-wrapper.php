...
class WC_Gift_Wrapping {
	...
	*/ 
	public function get_giftwrapped_products() {
		...
		// GEWIJZIGD: Zoek enkel naar voorradige cadeauverpakkingen
		$args = array(
			'post_type' => 'product',
			'post_status' => 'publish',
			'posts_per_page' => '-1',
	...
	*/ 
	public function giftwrap_list( $label = '' ) {
		...
		<div class="wc_giftwrap_notes_container">
			...
			<textarea name="wc_gift_wrap_notes" id="wc_gift_wrap_notes<?php echo $label ?>" cols="30" rows="2" maxlength="<?php echo get_option( 'giftwrap_textarea_limit' ); ?>" class="wc_giftwrap_notes"><?php if ( isset( WC()->session->gift_wrap_notes) ) { echo esc_textarea( WC()->session->gift_wrap_notes ); } ?></textarea>	
		</div>
	<?php } 
