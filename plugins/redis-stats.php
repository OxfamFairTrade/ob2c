...
// Changes text color of Redis memory usage
function warning_color($percentage) {
		$percentage = $percentage*100;
		if (($percentage >=  88) && ($percentage < 95)) {
			return 'DarkOrange';
...
var redisrefresh = getCookie('redisrefresh');

if( redisrefresh > 1 ) {
window.setTimeout(function(){ document.location.reload(true); }, redisrefresh*1000);
}
</script>
...
<form>
  ...
  <select name="interval" id="interval" onchange="window.setTimeout(function(){ document.location.reload(true); }, this.options[this.selectedIndex].value*1000); document.cookie = 'redisrefresh='+this.options[this.selectedIndex].value;">
    <option value="0">Disabled</option>
    <option value="5" <?php if (($_COOKIE['redisrefresh']) == 5) { echo 'selected'; } ?>>5</option>
    <option value="10" <?php if (($_COOKIE['redisrefresh']) == 10) { echo 'selected'; } ?>>10</option>
    <option value="15" <?php if (($_COOKIE['redisrefresh']) == 15) { echo 'selected'; } ?>>15</option>
		<option value="30" <?php if (($_COOKIE['redisrefresh']) == 30) { echo 'selected'; } ?>>30</option>
  </select>
</form>
...
<div class='box col'>
    <h2>Used Memory</h2>
    <div class="key"><font color="<? echo warning_color($data['used_memory']/$data['maxmemory']); ?>"><?php echo $data['used_memory_human'] ?></font></div>
    <h2>Peak Memory</h2>
    <div class="key"><font color="<? echo warning_color($data['used_memory_peak']/$data['maxmemory']); ?>"><?php echo $data['used_memory_peak_human'] ?></font></div>
</div>
