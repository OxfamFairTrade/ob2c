...
function relevanssi_date_queries($d, $title, $version = 'good') {
	...
	if (count($queries) > 0) {
		...
		foreach ($queries as $query) {
			...
			// GEWIJZIGD: laat logs rechtstreeks doorlinken naar specifieke product search in een nieuw venster
			echo "<tr><td style='padding: 3px 5px'><a href='$url/?s=$u_q&post_type=product' target='_blank'>" . esc_attr($query->query) . "</a></td><td style='padding: 3px 5px; text-align: center'>" . $query->cnt . "</td><td style='padding: 3px 5px; text-align: center'>" . $query->hits . "</td></tr>";
		}
		echo "</tbody></table>";
	}
}
