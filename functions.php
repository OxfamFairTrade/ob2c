<?php
	...
	function force_user_login() {
		...
		// Demosite afschermen TIJDELIJK UITSCHAKELEN
		if ( 1 === 2 and ( in_array( get_current_blog_id(), get_site_option('oxfam_blocked_sites') ) or get_current_site()->domain !== 'shop.oxfamwereldwinkels.be' ) ) {
	...
	function load_child_theme() {
		wp_enqueue_style( 'oxfam-webshop', get_stylesheet_uri(), array( 'nm-core' ), '1.5.15' );
	...
	function custom_listing_template() {
		...
		$listing_template .= "\t\t\t\t" . '<a href="/?addSku='.$_GET['addSku'].'"><button style="float: right;">Naar de webshop</button></a>' . "\r\n";
	...
	*/	
	...
	function print_store_selector() {
		...
		$msg .= '<h3 class="thuislevering">'.__( 'Vul je postcode in en ontvang je bestelling aan huis:', 'oxfam-webshop' ).'</h3><br/>';
